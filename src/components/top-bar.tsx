  ...
  return (
    <nav className="fixed left-0 right-0 top-0 z-50 w-full bg-white shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          <div className="flex">
            <div className="flex-shrink-0 flex items-center">
              <span className="text-gray-800 font-bold text-xl">Arman Khan</span>
            </div>
          </div>
        ...
        <div
          className={`md:hidden ${isOpen ? 'absolute' : 'hidden'} top-16 left-0 right-0 bg-white z-40`}>
...
const NavBarLink = ({title, href, extraClassNames = ''}: { title: string; href: string; extraClassNames?: string }) => {
  return (
    <Link href={href}
...
const NavBarLinks = ({mobile}: { mobile?: boolean }) => {
  return (
    <>
      <NavBarLink title='Home' href='/' extraClassNames={mobile ? 'block' : ''}/>
      <NavBarLink title='Blog' href='/blog' extraClassNames={mobile ? 'block' : ''}/>
      <NavBarLink title='Projects' href='/projects' extraClassNames={mobile ? 'block' : ''}/>
      <NavBarLink title='Contact' href='/contact' extraClassNames={mobile ? 'block' : ''}/>
    </>
  )
}
